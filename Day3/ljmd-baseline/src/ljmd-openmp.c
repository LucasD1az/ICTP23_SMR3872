...
#pragma omp parallel reduction(+:epot) private(i,j,rx,ry,rz,rsq,ffac) shared(sys,rcsq,c6,c12)
    ...
    */
    for(i=0; i < (sys->natoms-1); i+=sys->nthreads) {
        ...
        ry1=sys->ry[ii];
        rz1=sys->rz[ii];
        for(j=ii+1; j < (sys->natoms); ++j) { // I'm using Newton's 3rd law here

            //printf("These are particles %d and %d\n", ii, j);
...
1. The first line of the code calculates the number of atoms each thread will be responsible for.
2. The second line calculates the start index of the atoms that the current thread will be responsible for.
3. The third line calculates the end index of the atoms that the current thread will be responsible for.
4. The for loop iterates through all the threads and adds the force of the atoms that they are responsible for to the total force of the atoms that thread 0 is responsible for.
5. The for loop inside the for loop iterates through the atoms that the current thread is responsible for and adds the force of the atoms that the current thread is responsible for to the total force of the atoms that thread 0 is responsible for. */
