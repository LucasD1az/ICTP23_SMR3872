...
//    int nv = 0; // Now the code forces it to be 0
    int nv = NN/3;
    int mv = NM/3;
    double c = 0.5; // velocity
    double dx = 1.0 / (n - 1);
    double lambda = dx*n/10.0;
...
*/
    ...
    while (iter < iter_max )
    {
        ...
        for(i=1;i<m-1;i++){
            for(j=3;j>=0;j--){
                if(((i>=mv-3&&i<=mv+3)||(i>=m-mv-3&&i<=m-mv+3))) A[j][i]=1000*sin(dt*iter*w);
                else A[j][i]=A[j+1][i];
        ...
        for(j=1;j<n-1;j++){
            for(i=3;i>=0;i--){
                if(((j>=nv-3&&j<=nv+3)||(j>=n-nv-3&&j<=n-nv+3))) A[j][i]=1000*sin(dt*iter*w);
                else A[j][i]=A[j][i+1];
            //    A[j][i]=A[j][i+1];
            }
            for(i=m-4;i<m;i++){
        ...
        if(iter % 100 == 0&&iter<10000) //printf("%5d, %0.8lf\n", iter, error);
        {
