        ...
        // if(j>=nloc-1) break;
            for( i = 1; i < n-1; i++ ) {
                ...
                if(j_g>=nc-2&&j_g<nc+2&&i_g>=nc-2&&i_g<nc+2){  
                    //printf("I am %d and I am in the center! (%ld,%ld)\n",me,j,i);    
                    Anew[(j*n)+i]=1000*sin(dt*iter*w);
                    snew[(j*n)+i]=1000*w*cos(dt*iter*w);
                } else {
                    snew[(j*n)+i]=s[(j*n)+i]+0.5*c*((r[(j+1)*n+i]-r[(j-1)*n+i]+l[(j*n)+i+1]-l[(j*n)+i-1])+c*(s[(j+1)*n+i]-4*s[(j*n)+i]+s[(j-1)*n+i]+s[(j*n)+i+1]+s[(j*n)+i-1]));
...
void save_gnuplot( double *M, size_t dimension, int me, int ncpu, int nloc, int offset ){
  ...
  int next_n;
  double *Mrecv;
  Mrecv = (double *) malloc( dimension* (nloc+1) * sizeof(double) );
...
if(!me){
  ...
  for(i=1;i<ncpu;i++){
    ...
    MPI_Recv( Mrecv , dimension*(next_n-2) , MPI_DOUBLE , i , 0 , MPI_COMM_WORLD , MPI_STATUS_IGNORE);
    for(int j=0;j<next_n-2;j++){    
        for( int k = 0; k < dimension; ++k ) fprintf(file, "%f ", Mrecv[ j*dimension+k ] );
        fprintf(file, "\n" );
    }
    //printf("I am %d and I've just printed %ld!\n",me,i);
  }
  if(ncpu>1){
    //MPI_Recv (Mrecv, dimension, MPI_DOUBLE, ncpu-1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    //for( int k = 0; k < dimension; ++k ) fprintf(file, "%f ", Mrecv[ k ] );
  }
  ...
  fclose( file );
    ...
    MPI_Send( M + dimension , dimension*(nloc-2) , MPI_DOUBLE , 0 , 0 , MPI_COMM_WORLD );
}
if(me==ncpu-1&&ncpu>1){
    //MPI_Send( M + (nloc-1)*dimension , dimension , MPI_DOUBLE , 0 , 0 , MPI_COMM_WORLD );
}
